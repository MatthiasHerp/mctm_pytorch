...
class Multivariate_Transformation_Model(nn.Module):
    def __init__(self, polynomial_range, number_variables, degree_transformations=10, degree_decorrelation=10, monotonically_increasing=True):
    ...
    def forward(self, y, covariate=False, train=True, compute_optimal_initial_params=False):
        if train:
            output, log_d = self.l1.forward(y, return_log_d=True, compute_optimal_initial_params=compute_optimal_initial_params)
            return output, log_d, 0, 0, 0, 0
        else:
            output = self.l1.forward(y, return_log_d=False, compute_optimal_initial_params=compute_optimal_initial_params)
            return output
